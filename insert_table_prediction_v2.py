...
def max_check(currency, high, time_interval, currency_number, p_high):
    ...
    update_sql(sql)
    if max_high > p_high:
        time_sql = "UPDATE predicted_high_low SET date_time_hit_high = '" + str(datetime.utcnow()) + "' WHERE currency = '" + currency + "' and time_interval = '" + time_interval + "'"
        update_sql(time_sql)
...
def min_check(currency, low, time_interval, currency_number, p_low):
    ...
    update_sql(sql)
    if min_low < p_low:
        time_sql = "UPDATE predicted_high_low SET date_time_hit_low = '" + str(datetime.utcnow()) + "' WHERE currency = '" + currency + "' and time_interval = '" + time_interval + "'"
        update_sql(time_sql)
        ...
        if datetime_now < t_datetime:
            if time_ == "1Min":
                if not freeze_dict[currency][0]:
                    ...
                    query = "UPDATE predicted_high_low set current_price_high='" +str(current_price) +"'  WHERE currency = '" + currency + "' and time_interval = '" + time_ + "'"
                    update_sql(query)
